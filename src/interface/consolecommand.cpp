...
// (MYMOD async state moved to global scope at end of file)
	...
	static ConsoleCommand ccmd_aitest("/aitest", "MYMOD: talk to nearest monster via Ollama (async)", []CCMD{
		...
		if (!(svFlags & SV_FLAG_CHEATS)) { messagePlayer(clientnum, MESSAGE_MISC, Language::get(277)); return; }
		if (!players[clientnum] || !players[clientnum]->entity || !stats[clientnum]) { return; }
		stats[clientnum]->setProficiency(PRO_LEADERSHIP, 100);
		stats[clientnum]->CHR += 50;
		messagePlayer(clientnum, MESSAGE_MISC, "[MYMOD] Leadership=100, CHR now %d. Attack should be available; recruiting easier.", (int)statGetCHR(stats[clientnum], players[clientnum]->entity));
		});

static ConsoleCommand ccmd_aiforceattack("/aiforceattack", "MYMOD: directly order follower to attack nearest enemy (no AI)", []CCMD{
		if (!(svFlags & SV_FLAG_CHEATS)) { messagePlayer(clientnum, MESSAGE_MISC, Language::get(277)); return; }
		...
		Entity* follower = nullptr; double fbest = 1e18;
		for (auto node = map.entities->first; node != NULL; node = node->next) {
			auto e = (Entity*)node->element;
			if (e->behavior == &actMonster && e != pl) {
				Stat* es = e->getStats();
				if (es && es->leader_uid == myUID) {
					double dx=e->x-pl->x, dy=e->y-pl->y, d=dx*dx+dy*dy;
					if (d<fbest){fbest=d;follower=e;}
				}
			}
		}
		if (!follower) { messagePlayer(clientnum, MESSAGE_MISC, "[MYMOD] no follower"); return; }
		Entity* enemy = nullptr; double ebest = 1e18;
		for (auto node = map.entities->first; node != NULL; node = node->next) {
			auto e = (Entity*)node->element;
			bool attackable = (e->behavior == &actMonster && e != follower && follower->checkEnemy(e));
			if (attackable) {
				double dx=e->x-follower->x, dy=e->y-follower->y, d=dx*dx+dy*dy;
				if (d<ebest){ebest=d;enemy=e;}
			}
		}
		if (!enemy) { messagePlayer(clientnum, MESSAGE_MISC, "[MYMOD] no attackable target (enemy or door) nearby"); return; }
		const char* ttype = "monster";
		if (enemy->behavior == &actDoor) ttype = "door";
		else if (enemy->behavior == &actIronDoor) ttype = "iron door";
		double tdx = enemy->x - follower->x, tdy = enemy->y - follower->y;
		double tdist = sqrt(tdx*tdx + tdy*tdy) / 16.0;
		messagePlayer(clientnum, MESSAGE_MISC, "[MYMOD] forcing ATTACK: %s target uid=%u at %.1f tiles", ttype, (unsigned)enemy->getUID(), tdist);
		follower->monsterAllyInteractTarget = enemy->getUID();
		follower->monsterAllySendCommand(ALLY_CMD_ATTACK_SELECT, 0, 0, enemy->getUID());
		follower->monsterAllySendCommand(ALLY_CMD_ATTACK_CONFIRM, 0, 0, enemy->getUID());
		});

	void mymod_loadServerConfig();  // fwd
	static ConsoleCommand ccmd_aiserver("/aiserver", "MYMOD: set the AI backend URL, e.g. /aiserver http://192.168.1.50:5001", []CCMD{
		if (argc < 2) {
			messagePlayer(clientnum, MESSAGE_MISC, "[MYMOD] AI server is: %s", mymod_ai_server.c_str());
			messagePlayer(clientnum, MESSAGE_MISC, "[MYMOD] usage: /aiserver http://host:5001");
			return;
		}
